...
eval1 <- eval_dat[1:73]
eval2 <- eval_dat[74:146]

nn1<-nn2(newdat, eval1, k = 1)#k=min(10, nrow(wdat)), treetype = c("kd"))
nn2<-nn2(newdat, eval2, k = 1)#k=min(10, nrow(wdat)), treetype = c("kd"))
...
eval_res <- data.frame(0:(length(pairMatch)-1))
eval_res$new.col<-pairMatch * 1
colnames(eval_res) <- c("Id", "Prediction")
write.table(eval_res, file="pubfig_kaggle_eval_results_part2.txt", quote=FALSE, sep=",",  row.names = F)