...
for (wi in 1:10) {
	wtd<-createDataPartition(y=labels, p=.8, list=FALSE) #random .8
	ntrbx<-features[wtd, ] #.8*700 x 8 matrix of features
	ntrby<-labels[wtd] # features of respective wtd entries
	...
	ntebx<-features[-wtd, ] # testing features
	nteby<-labels[-wtd] # testing labels
